...
//
// Delivery is routed by lib/whatsapp.js: email contacts use Resend, WhatsApp
// respondents use Twilio WhatsApp, and other phone contacts use Twilio SMS.
// Mock mode remains available for local development and is always reported as
// simulated rather than pretending that a code left the server.

const bcrypt = require("bcryptjs");
const crypto = require("crypto");
const store = require("./store");
const { canonical: canonicalContact } = require("./contact");
const { getProvider: getMessagingProvider } = require("./whatsapp");
...
/**
 * Normalized contact, so "+234 801..." and "0801..." aren't treated as
 * different. Delegates to lib/contact.js -- the single definition used both
 * where a contact is written and where one is looked up. This used only to
 * strip spaces and punctuation, so "0801..." stayed "0801...", which Twilio
 * rejects and which never matched a bulk-invited "+234801..." account.
 */
function normalizeContact(contact) {
  return canonicalContact(contact);
}
 ...
 */
async function sendCode({ contact, respondentId, purpose = "contact_verification", studyName, requireDelivery = false }) {
  ...
  if (!result || !result.ok || (requireDelivery && result.simulated)) {
    ...
    const err = new Error(
      (result && result.error) || (result && result.simulated
        ? "Code delivery is not configured for this contact. Please contact the study team."
