...
class MockBrandDetectionProvider {
  async detect(media, brands) {
    db.prepare(
      `UPDATE media SET detection_status = 'unavailable', detection_provider = 'mock', detection_raw_json = ?
       WHERE id = ?`
    ).run(
      JSON.stringify({
...
class AzureVisionProvider {
  ...
  async detect(media, brands) {
    ...
    try {
      ...
      db.prepare(
        `UPDATE media SET detection_status = ?, detected_brand = ?, detection_provider = 'azure_vision', detection_raw_json = ?
         WHERE id = ?`
      ).run(
        match ? "done" : "unavailable",
      ...
      db.prepare(`UPDATE media SET detection_status = 'error', detection_provider = 'azure_vision', detection_raw_json = ? WHERE id = ?`).run(
