...
// spelling from study settings (lib/categories.js), the same way
// configuredBrandFor reconciles a detected brand against the configured
// candidate list -- an LLM read of "toothpaste" should land on the exact
// "Toothpaste" string the study is filtered/reported by, not a near-miss.
function configuredCategoryFor(label, categories) {
  const wanted = canonicalBrandName(label);
  if (!wanted) return null;
  return (categories || []).find((category) => canonicalBrandName(category) === wanted) || null;
}

// Best-effort category read for the Azure provider, which has no semantic
// classifier -- only OCR text off the packaging. Unlike brand OCR matching,
// there's no logo model to fall back on here, so this is the only category
// signal Azure can offer: does one (and only one) of the study's selected
// categories literally appear as a word on the pack.
function selectCategoryFromText({ textSignals, categories }) {
  if (!Array.isArray(categories) || !categories.length) return { category: null, method: null };
  const matches = categories.filter((category) => {
    const name = canonicalBrandName(category);
    return name.length >= 3 && (textSignals || []).some((signal) => canonicalBrandName(signal).includes(name));
  });
  const unique = [...new Set(matches)];
  return unique.length === 1 ? { category: unique[0], method: "ocr" } : { category: null, method: null };
}
...
class AzureVisionProvider {
  ...
  async identify(localPath, mediaType, mimeType, brands, categories) {
    ...
    const categoryMatch = selectCategoryFromText({ textSignals, categories });
    return { logos, textSignals, warnings, match, categoryMatch, minConfidence, minMargin };
  }
  async detect(media, brands, categories) {
    ...
    if (media.media_type === "audio") {
      try {
        await store.update("media", { id: media.id }, {
          ...
          detection_confidence: null, detection_method: null, detection_category: null,
    ...
    try {
      ...
      const { logos, textSignals, warnings, match, categoryMatch, minConfidence, minMargin } = await this.identify(local.path, media.media_type, null, brands, categories);
      await store.update(
        "media",
        { id: media.id },
        {
          ...
          detection_category: categoryMatch.category,
          detection_review_status: null,
          detection_verified_by: null,
          detection_verified_at: null,
          ...
          detection_raw_json: JSON.stringify({
            ...
            categoryMethod: categoryMatch.method,
            candidateCategories: categories || [],
            warnings,
...
class GeminiVisionProvider {
  ...
  async identify(localPath, mediaType, mimeType, brands, categories) {
    ...
    if (mediaType === "video") {
      ...
      try {
        for (const framePath of files) {
          const buf = fs.readFileSync(framePath);
          attempts.push(await identifyBrandWithGemini({ apiKey: this.apiKey, model: this.model, buffer: buf, mimeType: "image/jpeg", candidateNames, candidateCategories }));
        }
      } finally {
        cleanup();
      }
      ...
      const buf = fs.readFileSync(localPath);
      attempts.push(await identifyBrandWithGemini({ apiKey: this.apiKey, model: this.model, buffer: buf, mimeType: mimeType || "image/jpeg", candidateNames, candidateCategories }));
    }
    ...
    const detectedCategory = best && best.category ? configuredCategoryFor(best.category, candidateCategories) : null;
    return { status, detectedBrand: best ? best.brand : null, detectedCategory, confidence: best ? best.confidence : null, method: best ? "llm_vision" : null, attempts, candidateNames, candidateCategories, minConfidence };
  }
  async detect(media, brands, categories) {
    ...
    try {
      ...
      await store.update("media", { id: media.id }, {
        ...
        detection_raw_json: JSON.stringify({
          ...
          attempts: result.attempts.map((a) => a ? { brand: a.brand, category: a.category, confidence: a.confidence, reason: a.reason } : { unparsed: true }),
      ...
      return { status: result.status, detectedBrand: result.detectedBrand, detectedCategory: result.detectedCategory, confidence: result.confidence, method: result.method };
    } catch (e) {
      try {
        await store.update("media", { id: media.id }, {
  ...
  async detectFromTranscript(media, brands, categories) {
    try {
      ...
      const candidateNames = (brands || []).map((b) => b.name);
      const candidateCategories = categories || [];
      const result = await identifyBrandFromTranscript({ apiKey: this.apiKey, model: this.model, transcript, candidateNames, candidateCategories });
      const minConfidence = boundedNumber(process.env.BRAND_LOGO_MIN_CONFIDENCE, 0.7, 0, 1);
      ...
      return { status, detectedBrand: result ? result.brand : null, detectedCategory, confidence: result ? result.confidence : null, method: result && result.brand ? "llm_transcript" : null };
    } catch (e) {
      try {
        await store.update("media", { id: media.id }, {
          detection_status: "error", detection_provider: "gemini_vision",
...
async function identifyBrandInFile(filePath, mediaType, mimeType, brands, categories) {
  if (mediaType === "audio") return { status: "unavailable", detectedBrand: null, detectedCategory: null, confidence: null, method: null };
  let provider;
  try {
    provider = getProvider();
  } catch (e) {
    return { status: "unavailable", detectedBrand: null, detectedCategory: null, confidence: null, method: null };
  }
  if (typeof provider.identify !== "function") return { status: "unavailable", detectedBrand: null, detectedCategory: null, confidence: null, method: null };
  try {
    const result = await provider.identify(filePath, mediaType, mimeType, brands || [], categories || []);
    return { status: result.status, detectedBrand: result.detectedBrand, detectedCategory: result.detectedCategory, confidence: result.confidence, method: result.method };
  } catch (e) {
    return { status: "error", detectedBrand: null, detectedCategory: null, confidence: null, method: null };
  }
}
