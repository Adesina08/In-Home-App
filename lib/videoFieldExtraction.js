...
class AzureVideoFieldExtractionProvider {
  ...
  async analyze(videoFile, questions, brands) {
    ...
    try {
      ...
      // front-camera video it is most of it. Occasion, companions and reason
      // are spoken concepts -- OCR on a pack label will never surface them.
      //
      // Failure here is deliberately soft: no speech credentials, no audio
      // track, or a failed call all leave `transcript` empty and fall back to
      // vision-only pre-fill, exactly as before.
      transcript = await this.transcribeSpeech(videoFile.path);
      if (transcript) signals.push(transcript.toLowerCase());
      ...
      const sources = transcript ? "what you showed and what you said" : "on-screen text/labels";
      return {
