...
function generateReadSasUrl(blobName, expiryMinutes = 5) {
...
/** Persist bytes received from a trusted non-multipart source, such as Twilio. */
async function persistBuffer(buffer, mimetype = "application/octet-stream") {
  ...
  const filename = `${crypto.randomUUID()}${extensions[String(mimetype).toLowerCase()] || ".bin"}`;
  if (PROVIDER !== "azure_blob") {
    await fs.promises.mkdir(UPLOAD_DIR, { recursive: true });
    await fs.promises.writeFile(path.join(UPLOAD_DIR, filename), buffer, { flag: "wx", mode: 0o600 });
    return `/uploads/${filename}`;
  }
  const client = getContainerClient();
  await client.createIfNotExists();
  await client.getBlockBlobClient(filename).uploadData(buffer, { blobHTTPHeaders: { blobContentType: mimetype } });
...
async function deleteMedia(filePath) {
  if(filePath?.startsWith(BLOB_PREFIX)) { await getContainerClient().getBlockBlobClient(filePath.slice(BLOB_PREFIX.length)).deleteIfExists(); return; }
  await fs.promises.rm(path.join(UPLOAD_DIR,path.basename(filePath||"")),{force:true});
}

module.exports = { deleteMedia, persistUpload, persistBuffer, readMediaBuffer, materializeLocalFile, getMediaUrl, PROVIDER };
