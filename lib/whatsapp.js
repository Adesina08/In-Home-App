...
const sendgrid = require("./staffEmail");
...
class MockProvider {
  ...
  async send({ respondentId, to, template, variables, channel }) {
    let body = null;
    let error = null;
    try {
...
class TwilioProvider {
  ...
  async send({ respondentId, to, template, variables, channel }) {
    ...
    const form = new URLSearchParams({ To: this.address(trimmed, deliveryChannel), Body: body });
...
function sendgridError(error) {
  const providerErrors = error && error.response && error.response.body && error.response.body.errors;
  return Array.isArray(providerErrors) && providerErrors[0] && providerErrors[0].message
...
class RoutingProvider {
  get name() {
    ...
    return `${phoneName}${sendgrid.configured() ? "+sendgrid" : ""}`;
  }
  get isReal() {
    try { return getPhoneProvider().isReal || sendgrid.configured(); }
    catch (_) { return sendgrid.configured(); }
  }
  async send(args) {
    ...
    if (!sendgrid.configured()) return new MockProvider().send({ ...args, channel: "email" });
    try {
      const result = await sendgrid.sendRespondentMessage(args);
      await record({
      ...
      const message = sendgridError(error);
      let body = null;
      try { body = renderMessage(args.template, args.variables); } catch (_) {}
      await record({ respondentId: args.respondentId, template: args.template, to: args.to, variables: args.variables, provider: "sendgrid_email", status: "failed", body, error: message });
...
function isRealMessagingConfigured(contact) {
  if (contact && isEmail(contact)) return sendgrid.configured();
  try {
...
function messagingConfigError() {
  ...
  const emailError = sendgrid.configError();
  if (emailError) errors.push(emailError);
  return errors.length ? errors.join(" ") : null;
}

module.exports = { getProvider, isRealMessagingConfigured, messagingConfigError, providerName, resolveDeliveryChannel };
