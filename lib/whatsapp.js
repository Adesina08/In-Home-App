...
class MockProvider {
  ...
  async send({ respondentId, to, template, variables }) {
    ...
    await record({ respondentId, template, to, variables, provider: "mock", status: error ? "failed" : "simulated", body, error });
...
class TwilioProvider {
  ...
  async send({ respondentId, to, template, variables }) {
    ...
    try {
      ...
      await record({ respondentId, template, to, variables, provider: this.name, status: "failed", error: e.message });
    ...
    if (!/^\+[1-9]\d{6,14}$/.test(trimmed.replace(/^whatsapp:/, ""))) {
      ...
      await record({ respondentId, template, to, variables, provider: this.name, status: "failed", body, error });
    ...
    try {
      ...
      await record({ respondentId, template, to, variables, provider: this.name, status: "failed", body, error: diagnostic });
