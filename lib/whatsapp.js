...
class TwilioProvider {
  ...
  async send({ respondentId, to, template, variables, channel, contentSid, contentVariables, logBody }) {
    ...
    const useOtpTemplate = !contentSid && deliveryChannel === "whatsapp" && template === "otp_contact_verification" && this.whatsappOtpContentSid;
    ...
    if (contentSid) {
      ...
      // sent as a session reply, never a Meta-approved template.
      form.set("ContentSid", contentSid);
      form.set("ContentVariables", JSON.stringify(contentVariables || {}));
    } else if (useOtpTemplate) {
      // A code sent cold (no prior message from this contact) falls outside
      // Meta's 24h freeform window, so it must go through an approved
      // authentication template -- see the constructor comment.
      form.set("ContentSid", this.whatsappOtpContentSid);
      form.set("ContentVariables", JSON.stringify({ "1": String(variables.code) }));
    } else {
      form.set("Body", body);
    }
