...
async function loadStudyReport(studyId, period = {}) {
  ...
  const records = await store.find('diary_records', { study_id: studyId, is_practice: 0, status: 'submitted', ...((from || to) ? { entry_time: periodFilter(from, to) } : {}) }, { sort: { entry_time: -1, id: -1 } });
  ...
  const answers = rawAnswers.filter(a => !blocked.has(a.record_id) && confirmed(a) && questionMap.has(a.question_id));
  ...
  const media = rawMedia.filter(m => !blocked.has(m.record_id) && ['photo', 'video', 'audio'].includes(m.media_type)).map(m => ({ id: m.id, media_type: m.media_type, file_path: m.file_path, day: String(byRecord.get(m.record_id).entry_time || '').slice(0, 10) }));
  ...
  const days = new Map();
  records.forEach(r => { const day = String(r.entry_time).slice(0, 10); days.set(day, (days.get(day) || 0) + 1); });
  ...
  return {
    ...
    words: wordCloud(openText.map(a => a.answer)), brands: distribution('brand'), occasions: distribution('occasion'),
