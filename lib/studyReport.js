...
async function loadStudyReport(studyId, period = {}) {
  ...
  const openText = answers
    .filter(a => ['text', 'longtext'].includes(questionMap.get(a.question_id).type) && a.value != null && String(a.value).trim())
    .map(a => ({ question: questionMap.get(a.question_id).text, answer: String(a.value) }));
  for (const answer of answers) {
    if (!answer.other_text_json) continue;
    try { for (const [option, text] of Object.entries(JSON.parse(answer.other_text_json))) if (String(text || '').trim()) openText.push({ question: `${questionMap.get(answer.question_id).text} — ${option}`, answer: String(text).trim() }); } catch (_) {}
  }
  const byRecord = new Map(records.map(r => [r.id, r]));
  const media = rawMedia.filter(m => !blocked.has(m.record_id) && ['photo', 'video', 'audio'].includes(m.media_type)).map(m => ({
    id: m.id, record_id:m.record_id, media_type: m.media_type, file_path: m.file_path, question_id: m.question_id ?? null,
    day: String(byRecord.get(m.record_id).occurrence_time || byRecord.get(m.record_id).entry_time || '').slice(0, 10),
    transcript_status: m.transcript_status || null, transcript_text: m.transcript_text || null,
    detection_status: m.detection_status || null, detected_brand: m.detected_brand || null, detected_category: m.detection_category || null, detection_confidence: m.detection_confidence ?? null,
  }));
  const wordCloudTexts = openText.map(a => a.answer).concat(media
    .filter(m => ['audio', 'video'].includes(m.media_type) && m.transcript_status === 'done' && String(m.transcript_text || '').trim())
  ...
  return {
    ...
    words: wordCloud(wordCloudTexts), brands: analytics.brands.map(b=>({label:b.brand,n:b.occasions})), occasions: distribution('occasion'),
