...
function runQcForRecord(recordId) {
  ...
  // 6. Required fields missing (photo/video questions are evidence, tracked via the media table, not responses)
  const requiredQs = db
    .prepare("SELECT id, text FROM questions WHERE study_id = ? AND required = 1 AND active = 1 AND type NOT IN ('photo', 'video')")
    .all(study.id);
...
module.exports = { runQcForRecord, checkCrossChannelDuplicate, classifyRisk, raiseFlag };
