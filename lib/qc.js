...
async function runQcForRecord(recordId) {
  ...
  if (study.qc_back_entry_enabled && record.occurrence_time && record.entry_time) {
    const gap = hoursBetween(record.occurrence_time, record.capture_time || record.entry_time);
    if (gap > study.back_entry_hours) {
      await raiseFlag(
        ...
        `Entry captured ${gap.toFixed(1)}h after occurrence; allowed window is ${study.back_entry_hours}h`
      );
    }
  }
 ...
 *
 *   compliance (35%)  -- completed eligible participation periods / expected periods
 ...
 *   media response (15%) -- AI transcript scores where a real transcript and
 *                        scoring model ran; otherwise successful media
 *                        processing is 100 and actual processing errors are 0.
 *                        "Unavailable" providers are excluded, never penalised.
 *
...
async function computeQualityScore(respondentId) {
  ...
  const report=await require("./studyReport").loadStudyReport(study.id);
  const participation=require("./researchMetrics").participation(study,[respondent],report.eligibleRecords.filter(r=>r.respondent_id===respondentId));
  const expected=participation.expected;
  const complianceScore=participation.rate;
  ...
  const mediaSignals = media.map((m) => {
    if (m.transcript_score_status === "done" && Number.isFinite(Number(m.transcript_score))) {
      return Math.max(0, Math.min(100, Number(m.transcript_score)));
    }
    if (m.transcript_status === "error" || m.detection_status === "error" || m.transcript_score_status === "error") return 0;
    if (m.transcript_status === "done" || m.detection_status === "done") return 100;
    return null;
  }).filter((value) => value !== null);
  const mediaScore = mediaSignals.length ? Math.round(mediaSignals.reduce((sum, value) => sum + value, 0) / mediaSignals.length) : 100;

  const score = Math.round(
    ((complianceScore===null?0:complianceScore*0.35) + flagScore * 0.35 + followThroughScore * 0.15 + mediaScore * 0.15)/(complianceScore===null?0.65:1)
  );
  ...
  return {
    score,
    band,
    breakdown: { complianceScore, flagScore, followThroughScore, mediaScore, transcriptScoredCount: media.filter((m) => m.transcript_score_status === "done").length, expected, submitted, drafts, openFlagCount: openFlags.length },
  };
}
