...
const mediaMigrations = [
  ...
  // Shown on the invite landing page, so a cold-texted respondent can decide
  // whether to take part before being asked for anything. Without it the page
  // would open with a consent form and no explanation of what the study is.
  "ALTER TABLE studies ADD COLUMN invite_brief TEXT",
  // When the invite SMS went out, and which mode the respondent picked on the
  // landing page -- so recruitment can be reported on rather than guessed at.
  "ALTER TABLE respondents ADD COLUMN invite_sent_at TEXT",
  "ALTER TABLE respondents ADD COLUMN chosen_mode TEXT",
];
