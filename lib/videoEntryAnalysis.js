...
async function analyzeSubmittedVideo({ recordId, videoFile, questions, brands, categories, studyVersion }) {
  let result;
  let media;
  try {
    media=await store.findOne("media",{record_id:recordId,media_type:"video"});
    const local=media?await require("./mediaStorage").materializeLocalFile(media.file_path):{path:videoFile.path,cleanup:()=>{}};
    try{result = await getProvider().analyze({...videoFile,path:local.path}, questions, brands);}finally{local.cleanup();}
  } catch (e) {
    result = { status: "error", prefill: {}, transcript: "", note: `AI video review failed: ${e.message}` };
  }

  // Capture-first Video mode used to run field extraction but never update the
  // media row's brand-detection fields. Run the dedicated logo provider too so
  // photo questions, ordinary video evidence and Video-mode entries all use
  // the same confidence-gated brand result.
  if (media) {
    try {
      await require("./brandDetection").getProvider().detect(media, brands, categories);
    } catch (error) {
      console.warn(`Video brand detection failed for record ${recordId}: ${error.message}`);
    }
  }
  ...
  await store.update("diary_records",{id:recordId,review_status:"pending_ai"},{review_status:"pending"});
