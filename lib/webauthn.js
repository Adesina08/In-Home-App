...
async function getCredentialsForRespondent(respondentId) {
  return store.find("respondent_credentials", { respondent_id: respondentId });
}

async function buildRegistrationOptions(req, respondent) {
  const existing = await getCredentialsForRespondent(respondent.id);
...
async function verifyRegistration(req, respondent, responseJSON) {
  ...
  await store.insert("respondent_credentials", {
    respondent_id: respondent.id,
    credential_id: credential.id,
    public_key: isoBase64URL.fromBuffer(credential.publicKey),
    counter: credential.counter,
    device_label: (req.get("user-agent") || "").slice(0, 120),
  });
  return true;
}
...
async function verifyAuthentication(req, respondent, responseJSON) {
  ...
  const stored = await store.findOne("respondent_credentials", {
    respondent_id: respondent.id,
    credential_id: responseJSON.id,
  });
