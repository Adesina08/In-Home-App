...
function validateSubmission({ questions, rules, body }) {
  questions=questions.filter(q=>require("./advancedQuestionnaire").scheduled(q,{occurrenceTime:body.occurrence_time||new Date().toISOString(),occasionNumber:Number(body.occasion_number)||1}));
  const errors = [];
  const answers = answersFrom(questions, body);
  const visible = visibleQuestionIds(questions, rules || [], answers);
  let otherText = body.other_text_json || {};
  if (typeof otherText === "string") { try { otherText = JSON.parse(otherText); } catch (_) { otherText = {}; } }
  ...
  questions.forEach((q) => {
    ...
    if (q.type === "date") { if(!/^\d{4}-\d{2}-\d{2}$/.test(raw)||!Number.isFinite(Date.parse(raw))||new Date(raw).toISOString().slice(0,10)!==raw)push("Choose a valid calendar date."); return; }
    if (q.type === "time") { if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(raw))push("Use a valid 24-hour time (HH:mm)."); return; }
    if (q.type === "numeric" || q.type === "scale") {
      ...
      // A per-question increment (e.g. 10, so a price question refuses 761 --
      // prices don't come in pennies here). Measured from the question's own
      // minimum where one is set, otherwise from zero, with a small epsilon
      // since the answer arrives as a string run through parseFloat.
      const step = q.step_value;
      if (step !== null && step !== undefined && Number(step) > 0) {
        const stepN = Number(step);
        const base = min !== null && min !== undefined ? Number(min) : 0;
        const ratio = (n - base) / stepN;
        if (Math.abs(ratio - Math.round(ratio)) > 1e-9) {
          push(base
            ? `Please enter a number in steps of ${stepN} starting from ${base}.`
            : `Please enter a number in steps of ${stepN}.`);
          return;
        }
      }
      return;
    ...
    if (["single","multi","rank"].includes(q.type)) {
      ...
      let options = [];
      try {
        options = require("./questionnaire").parseOptions(q.options_json||q.options);
      } catch (e) {
        return; // malformed config is not the respondent's problem
      }
      if (!options.length) return;
      const chosen = ["multi","rank"].includes(q.type) ? String(raw).split("|").filter(Boolean) : [String(raw)];
      if (q.type === "multi") {
        const maxSelections = Number(q.max_selections);
        if (Number.isInteger(maxSelections) && maxSelections > 0 && chosen.length > maxSelections) {
          push(`Select up to ${maxSelections} option${maxSelections === 1 ? "" : "s"}.`);
          return;
        }
      }
      if(q.type === "rank" && (new Set(chosen).size!==options.length||chosen.length!==options.length))push("Rank every listed option exactly once.");
      const unknown = chosen.filter((c) => !options.includes(c));
      if (unknown.length) push("Please choose one of the listed options.");
      const specify = require("./questionnaire").parseOptions(q.other_specify_options_json || q.otherSpecifyOptions);
      for (const option of chosen.filter((value) => specify.includes(value))) {
        const value = otherText[String(q.id)] && otherText[String(q.id)][option];
        if (!String(value || "").trim()) push(`Please specify your answer for “${option}”.`);
      }
    }
  });
