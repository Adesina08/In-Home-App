...
// The three banners a "simple crosstab" is expected to show on the AI
// analysis page. A subset of SEGMENTS, in display order.
const BANNER_SEGMENTS = { location: 'Location', gender: 'Gender', age_band: 'Age' };
...
/** Question families (a base question plus any loop repeats of it) available for crosstabbing, and the one selected. */
function selectQuestion(report, question) {
...
/** Eligible answers to the selected question, one merged set of option values per record. */
function answersForQuestion(report, selectedQuestion) {
  ...
  return { records, answerByRecord };
}
/** Groups respondents by one demographic segment and tallies their answers into a crosstab against a shared distribution. */
async function groupBySegment(report, records, answerByRecord, segment, profiles) {
  ...
  return { distribution, crosstab, minimum, answered: answerByRecord.size, answerContributors: contributors.size };
}
...
async function staffAnalysis(studyId, { from = '', to = '', question = '', segment = 'gender' } = {}) {
  ...
  const { questions, selectedQuestion } = selectQuestion(report, question);
  const profiles = await loadProfiles(studyId);
  const { records, answerByRecord } = answersForQuestion(report, selectedQuestion);
  const { distribution, crosstab, minimum, answered, answerContributors } = await groupBySegment(report, records, answerByRecord, segment, profiles);
  const days = new Map();
  ...
  return { report, questions, question: selectedQuestion?.code || '', questionLabel: selectedQuestion?.text || 'No structured questions available', segment, segments: SEGMENTS, distribution, crosstab, minimum, answered, answerContributors,
    trend: [...days].sort(([a], [b]) => a.localeCompare(b)).map(([label, n]) => ({ label, n })), modes: [...modes].map(([label, n]) => ({ label, n })) };
}
/**
 * A simple crosstab of one questionnaire question against the three
 * standard demographic banners (Location, Gender, Age) side by side --
 * the shared report, question list and profile lookup are loaded once and
 * reused for all three, since the answer distribution itself doesn't
 * depend on which banner it's being broken out by.
 */
async function bannerAnalysis(studyId, { from = '', to = '', question = '' } = {}) {
  const report = await loadStudyReport(studyId, { from, to });
  const { questions, selectedQuestion } = selectQuestion(report, question);
  const profiles = await loadProfiles(studyId);
  const { records, answerByRecord } = answersForQuestion(report, selectedQuestion);
  const banners = {};
  for (const key of Object.keys(BANNER_SEGMENTS)) banners[key] = await groupBySegment(report, records, answerByRecord, key, profiles);
  return { report, questions, question: selectedQuestion?.code || '', questionLabel: selectedQuestion?.text || 'No structured questions available', banners, bannerLabels: BANNER_SEGMENTS };
}
...
module.exports = { staffAnalysis, bannerAnalysis, analysisCsv, SEGMENTS, BANNER_SEGMENTS };
