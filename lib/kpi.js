...
async function loadEntries(studyId) {
  return (await require('./studyReport').loadStudyReport(studyId)).entries;
}
 ...
 */
function computeKpi(kpi, entries, {respondentIds}={}) {
  ...
  if (kpi.metric === "percent_respondents") {
    const allPeople = new Set(respondentIds || entries.map((e) => e.respondentId));
  ...
  if (kpi.metric === "average") {
    const nums = matching
      .filter(e=>e.answers[kpi.question_id]!==undefined&&e.answers[kpi.question_id]!==null&&String(e.answers[kpi.question_id]).trim()!=="")
...
async function computeAll(studyId, kpis) {
  const report=await require("./studyReport").loadStudyReport(studyId);
  const entries=report.entries;
  const out = new Map();
  kpis.forEach((k) => out.set(k.id, computeKpi(k, entries,{respondentIds:report.respondents.map(r=>r.id)})));
