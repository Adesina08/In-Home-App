 ...
 */
async function loadEntries(studyId) {
  const records = await store.find(
    "diary_records",
    { study_id: studyId, status: "submitted", is_practice: 0 },
    { sort: { id: 1 }, projection: { id: 1, respondent_id: 1 } }
  );
  if (!records.length) return [];

  const byId = new Map(records.map((r) => [r.id, { recordId: r.id, respondentId: r.respondent_id, answers: {} }]));
  // The responses -> diary_records join is done in JS: the records above are
  // already exactly the submitted, non-practice ones, so their ids are the
  // join. Rows are read in id order, so where the same question appears twice
  // for a record the last one still wins, as it did on the SQLite scan.
  const rows = await store.find(
    "responses",
    { record_id: { $in: [...byId.keys()] } },
    { sort: { id: 1 }, projection: { record_id: 1, question_id: 1, value: 1 } }
  );
  rows.forEach((row) => {
...
async function computeAll(studyId, kpis) {
  const entries = await loadEntries(studyId);
  const out = new Map();
