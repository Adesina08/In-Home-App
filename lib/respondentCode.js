...
function nextRespondentCode(studyId) {
  const prefix = `R${String(studyId).padStart(2, "0")}-`;

  const rows = db.prepare("SELECT respondent_code FROM respondents WHERE study_id = ?").all(studyId);
  let highest = 0;
  rows.forEach((r) => {
  ...
  const exists = db.prepare("SELECT 1 FROM respondents WHERE respondent_code = ?");
