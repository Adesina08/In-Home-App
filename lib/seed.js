...
// Day-1 inputs: questionnaire, consent wording, thresholds and KPIs.

const bcrypt = require("bcryptjs");
const { v4: uuidv4 } = require("uuid");
const store = require("./store");
const respondentAccounts = require("./respondentAccounts");
...
async function wipe() {
  ...
  //
  // This list used to cover only the collections the original seed wrote to.
  // Everything the app has grown since (fieldwork, incentives, reporting,
  // research ops, mobile sessions, ...) was left behind on every reseed, so a
  // "clean" database still had leftover rows pointing at studies/respondents
  // that no longer existed. Kept in sync with lib/store/schema.js's DEFAULTS
  // plus the handful of collections that carry no field defaults.
  const collections = [
    ...
    "questionnaire_versions", "consent_versions", "users", "studies", "brands",
    "respondent_credentials", "push_subscriptions", "ai_summaries", "respondent_accounts", "otp_codes",
    "password_reset_tickets", "staged_media", "system_settings",
    "respondent_profiles", "respondent_profile_snapshots", "whatsapp_sessions", "whatsapp_inbound_messages",
    "end_validations", "follow_up_log", "mobile_sessions",
    "diary_submissions", "interviewer_assignments", "fieldwork_visits",
    "incentive_rules", "incentive_ledger", "client_grants",
    "report_schedules", "report_snapshots", "research_alerts", "research_audit",
    "backchecks", "privacy_requests", "theme_codes", "coded_verbatims",
    "counters",
  ];
...
async function seed() {
  ...
  await store.update("questions", { id: brandQ }, { other_specify_options_json: JSON.stringify(["Other"]) });
  ...
  const seedStudy = await store.findOne("studies", { id: studyId });
  const seededBrands = await require("./productCandidates").forStudy(seedStudy);
  ...
  await getBrandDetectionProvider().detect({ id: videoMedia.id }, seededBrands, require("./categories").parseCategories(seedStudy.category));
  ...
  // Demo mobile respondent login (expo-mobile app) -- lets the app be logged
  // into immediately after a seed, without going through the real invite/OTP
  // activation flow. Linked to active[0] (Blessing Adeyemi, R01-0005), who
  // already has a submitted video-mode diary entry so Home/Diary/Activity
  // show real data instead of an empty state.
  const demoAccount = await respondentAccounts.findOrCreate({
    contact: "+2347000005000",
    name: "Blessing Adeyemi",
  });
  await respondentAccounts.setCredentials(demoAccount.id, { username: "demo", password: "Demo1234!" });
  await store.update("respondents", { id: active[0].id }, { account_id: demoAccount.id });

  console.log("Seed complete.");
  console.log("Study ID:", studyId);
  console.log("Demo mobile login (expo-mobile app): username 'demo', password 'Demo1234!'");
