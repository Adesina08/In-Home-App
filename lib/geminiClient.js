...
async function identifyBrand({ apiKey, model, buffer, mimeType, candidateNames }) {
  const hint = Array.isArray(candidateNames) && candidateNames.length
    ...
    : "";
  const prompt =
    `You are reviewing a market-research diary photo of a food or drink product, or its packaging.${hint}\n` +
    ...
    `Respond with ONLY strict JSON, no markdown fences: {"brand": "<the brand name exactly as printed, or null if none is legible>", "confidence": <0 to 1>, "reason": "<one short sentence>"}`;
...
async function identifyBrandFromTranscript({ apiKey, model, transcript, candidateNames }) {
  const hint = Array.isArray(candidateNames) && candidateNames.length
    ? ` This study is tracking these brands in particular, so prefer one of them if it matches what was said: ${candidateNames.join(", ")}.`
    : "";
  const prompt =
    `Below is a transcript of a market-research diary voice note, where someone describes a food or drink they consumed.${hint}\n` +
    ...
    `Respond with ONLY strict JSON, no markdown fences: {"brand": "<the brand name they said, or null if none was named>", "confidence": <0 to 1>, "reason": "<one short sentence>"}`;
