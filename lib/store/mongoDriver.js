...
function isCosmosOrderByIndexError(error) {
  ...
  // Cosmos rejects an unsupported ORDER BY with two different messages, and we
  // must recognise BOTH or the in-memory fallback below never runs:
  //
  //   single field, field not indexed:
  //     "The index path corresponding to the specified order-by item is excluded."
  //   two or more fields, no composite index:
  //     "The order by query does not have a corresponding composite index that
  //      it can be served from."
  //
  // Only the first was matched originally, so every multi-key sort in the app
  // -- { order_index: 1, id: 1 } and friends -- threw a 500 instead of falling
  // back to the in-memory sort. Match "order by"/"order-by" plus any of the
  // index complaints.
  const isOrderBy = message.includes("order-by") || message.includes("order by");
  if (!isOrderBy) return false;
  return (
    message.includes("composite index") ||
    (message.includes("index path") && message.includes("excluded")) ||
    message.includes("index is excluded")
  );
}
