...
function create({ file }) {
  ...
  async function insert(name, doc) {
    return withLock(async () => {
      ...
      if (coll(name).some(row => row._id === id)) { const error = new Error("Duplicate document id"); error.code = 11000; throw error; }
      const { id: _ignored, ...rest } = doc;
      coll(name).push({ _id: id, ...rest });
      if (typeof doc.id === "number" && (counters[name] || 0) < id) counters[name] = id;
      persist();
      return { id };
    });
  }
