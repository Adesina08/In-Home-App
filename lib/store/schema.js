// What a document looks like when a field wasn't supplied.
//
// SQLite applied these as column DEFAULTs, so a great deal of the app inserts
// partial rows and relies on the database to fill in the rest -- a respondent
// created without `consent_status` came back as "pending", an entry without
// `status` as "draft". MongoDB has no such concept: the field would simply be
// absent, every `status = 'draft'` filter would quietly stop matching it, and
// nothing would throw. That is the single most dangerous difference between the
// two stores, so the defaults are declared here and applied on every insert.
//
// `NOW` means "the moment of insertion", formatted the way the rest of the app
// stores time (see ../store/index.js).

const NOW = Symbol("now");

const DEFAULTS = {
  studies: {
    status: "draft",
    diary_mode: "daily",
    recruitment_mode: "hybrid",
    back_entry_hours: 24,
    recall_window_hours: 48,
    mandatory_photo: 1,
    duplicate_similarity_threshold: 0.9,
    burst_entry_count_threshold: 3,
    burst_entry_window_hours: 2,
    default_reminder_channel: "whatsapp",
    version: 1,
    questionnaire_dirty: 0,
    qc_duplicate_enabled: 1,
    qc_burst_enabled: 1,
    qc_back_entry_enabled: 1,
    created_at: NOW,
  },
  // must_change_password: set when an admin issues a temporary password, and
  // cleared once the person picks their own. Defaulted here rather than left
  // absent, because a missing field would not match the filter that looks for
  // one -- the schemaless trap this file exists to close.
  users: { created_at: NOW, must_change_password: 0, invite_delivery_status: "activated" },
  // Media uploaded while the respondent is still filling the form, before any
  // diary_record exists to attach it to. `consumed_at` stays null until a
  // submit claims it; anything still null after a day is an abandoned entry
  // and can be swept. Defaults declared here because a missing consumed_at
  // would drop the row out of every filter looking for one.
  staged_media: { created_at: NOW, consumed_at: null },
  brands: { active: 1 },
  consent_versions: { status: "draft", created_at: NOW },
  // applicable_cadences: null/empty means "every cadence" -- the default, so
  // every question in every existing study keeps behaving exactly as before
  // this feature existed. Only set when an admin deliberately scopes a
  // question to specific diary_mode values (e.g. a period-frequency question
  // that only makes sense on a weekly or monthly study).
  questions: { order_index: 0, required: 1, active: 1, applicable_cadences: null, max_selections: null },
  questionnaire_versions: { created_at: NOW },
  skip_rules: { operator: "equals", action: "show" },
  kpi_config: { enabled: 1 },
  respondents: {
    preferred_channel: "app",
    consent_status: "pending",
    activation_status: "invited",
    is_practice: 0,
    biometric_exempt: 0,
    created_at: NOW,
    // null: not yet due. "pending": the study period has ended (or an admin
    // triggered it manually) and the respondent must complete the close-out
    // questionnaire before logging anything further. "completed": done.
    end_validation_status: null,
  },
  // One row per respondent per completed close-out. Kept as its own
  // collection, not extra columns on `respondents`, so the answers and the
  // exact wording/version they answered stay together and auditable, the same
  // reasoning as consent_versions.
  end_validations: { created_at: NOW },
  // One row per exception follow-up contact attempt (spec 10.5 step 4: phone
  // calls concentrated on respondents with unresolved late/incomplete/
  // inconsistent/unclear diaries). Logging an attempt does NOT resolve the
  // underlying QC flags -- that stays a deliberate QC decision on the flag
  // itself -- it only records that contact was attempted, so the queue can
  // avoid calling the same person twice in one day and so there is an audit
  // trail of who was reached and what was found.
  follow_up_log: { created_at: NOW },
  // Platform-wide operational settings, set from the app itself rather than
  // requiring Azure App Service access. A single row (lib/systemSettings.js
  // always reads/creates the first one). scheduler_interval_minutes controls
  // how often the reminder engine and end-of-diary trigger check for
  // anything due -- previously fixed by an environment variable a study
  // owner had no way to reach.
  system_settings: { scheduler_interval_minutes: 15, updated_at: NOW },
  respondent_profiles: {
    recontact_consent: null,
    created_at: NOW,
    updated_at: NOW,
  },
  respondent_profile_snapshots: { captured_at: NOW },
  whatsapp_sessions: { step: "start", created_at: NOW, updated_at: NOW },
  whatsapp_inbound_messages: { created_at: NOW },
  diary_records: {
    entry_time: NOW,
    channel: "app",
    status: "draft",
    is_practice: 0,
    entry_mode: "standard",
  },
  // source: "respondent" for an answer a person actually gave, "ai_video" for
  // one the video extractor derived. verified: 0 until a researcher confirms
  // it. Defaults matter here -- an unset `source` would silently drop the row
  // out of every filter looking for one (see the schemaless-store note above),
  // and an AI guess that reads as respondent-given is exactly the confusion
  // this field exists to prevent.
  responses: { source: "respondent", verified: 1 },
  media: {
    media_type: "photo",
    upload_time: NOW,
    qc_status: "pending",
    detection_status: "not_run",
    detection_confidence: null,
    detection_method: null,
    detection_category: null,
    detection_review_status: null,
    transcript_status: "not_run",
    transcript_score_status: "not_run",
  },
  question_imports: { created_at: NOW },
  qc_flags: { severity: "medium", created_time: NOW, status: "open" },
  reminders: { channel: "whatsapp", scheduled_time: NOW, status: "scheduled" },
  whatsapp_outbox: { provider: "mock", status: "simulated", created_at: NOW },
  audit_log: { created_at: NOW },
  respondent_credentials: { counter: 0, created_at: NOW },
  push_subscriptions: { created_at: NOW },
  ai_summaries: { base_records: 0, base_respondents: 0, used_ai_model: 0, generated_at: NOW },
  respondent_accounts: { created_at: NOW },
  otp_codes: { purpose: "contact_verification", attempts: 0, created_at: NOW },
  password_reset_tickets: { consumed_at: null, created_at: NOW },
};

/**
 * Fill in whatever the caller left out.
 *
 * Only genuinely absent fields are filled: an explicit null is a deliberate
 * "no value" and is left alone, exactly as SQLite treated an explicit NULL.
 */
function applyDefaults(collection, doc, now) {
  const defaults = DEFAULTS[collection];
  if (!defaults) return doc;
  const out = { ...doc };
  for (const [field, value] of Object.entries(defaults)) {
    if (out[field] === undefined) out[field] = value === NOW ? now : value;
  }
  return out;
}

module.exports = { DEFAULTS, applyDefaults, NOW };
