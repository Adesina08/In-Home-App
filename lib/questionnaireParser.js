...
function normalizeRow(raw, idx) {
  ...
  if (conditionRaw) {
    warnings.push(`Condition "${conditionRaw}" was captured but not applied — set up this skip rule manually on the Skip Logic tab after committing.`);
  }
  return {
    row: idx + 1,
...
function parseProse(text) {
  ...
  function flush() {
    ...
    if (current.conditionRaw) {
      rowWarnings.push(`Condition "${current.conditionRaw}" was captured but not applied — set up this skip rule manually on the Skip Logic tab after committing.`);
    }
    rows.push({
      row: rows.length + 1,
      code: current.code,
