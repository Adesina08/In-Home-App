...
const VALID_TYPES = ["single", "multi", "numeric", "scale", "rank", "text", "date", "time", "photo", "video", "audio"];
...
const TEMPLATE_TYPE_TOKENS = [
  ...
  "single-choice", "multi-select", "numeric", "number", "scale", "rank", "text", "date", "time", "matrix", "photo", "video",
...
function splitOptions(raw) {
  if (!raw) return [];
  const delimiter = String(raw).includes("|") ? /\|/ : /[;,]/;
  return String(raw)
    .split(delimiter)
    .map((o) => o.trim())
    .filter(Boolean);
...
function normalizeRow(raw, idx) {
  ...
  const otherSpecifyOptions = splitOptionsPipeOnly(get("other specify options", "other_specify_options", "other specify"));
  const terminateEntryOptions = splitOptionsPipeOnly(get("terminate entry options", "terminate_entry_options"));
  const terminateStudyOptions = splitOptionsPipeOnly(get("terminate study options", "terminate_study_options"));
  for (const value of [...otherSpecifyOptions, ...terminateEntryOptions, ...terminateStudyOptions]) {
    if (!options.includes(value)) warnings.push(`Behaviour option "${value}" is not present in Options.`);
  }
  for (const value of terminateEntryOptions) if (terminateStudyOptions.includes(value)) warnings.push(`Option "${value}" cannot end both an entry and the study.`);
  ...
  return {
    ...
    other_specify_options: otherSpecifyOptions,
    terminate_entry_options: terminateEntryOptions,
    terminate_study_options: terminateStudyOptions,
    warnings,
  };
}
