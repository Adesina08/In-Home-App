...
const { transcribeLocalAudio, isSpeechConfigured, providerName } = require("./localAudioTranscription");
const { scoreTranscript, providerName: scoringProviderName } = require("./transcriptScoring");
...
async function analyseLocalMedia({ filePath, mediaType, question }) {
  ...
  try {
    ...
    return {
      ...
      scoreProvider: scored.provider || null,
    };
  } catch (error) {
    return {
...
async function analyseStoredMedia(media, question) {
  ...
  try {
    ...
    await store.update("media", { id: media.id }, {
      ...
      transcript_provider: result.transcriptStatus === "done" ? providerName() : null,
      transcript_score_status: result.scoreStatus,
      transcript_score: result.score,
      transcript_score_reason: result.scoreRationale,
      transcript_score_provider: result.scoreStatus === "done" ? (result.scoreProvider || scoringProviderName()) : null,
    });
