...
const { splitPresurvey } = require("./presurveySections");
...
async function loadQuestionnaire(studyId, context = {}, options = {}) {
  ...
  const scoped = cadence
    ...
    : allQuestions;
  // A respondent's diary is separate from the invitation screener. Keep the
  // published snapshot intact for the invitation and admin preview, but never
  // send pre-survey questions (or their skip rules) to a diary client.
  const respondentDiary = context.respondentId != null && !options.draft;
  const visible = respondentDiary ? splitPresurvey(scoped).diary : scoped;
  const visibleIds = new Set(visible.map((q) => Number(q.id)));
  const rules = (payload.rules || []).filter((rule) => !respondentDiary ||
    ((rule.condition_question_id == null || visibleIds.has(Number(rule.condition_question_id))) &&
     (rule.target_question_id == null || visibleIds.has(Number(rule.target_question_id)))));

  return {
    questions: require("./advancedQuestionnaire").contextualize(visible, context),
