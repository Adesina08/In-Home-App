...
function clientSafeReport(report, grant) {
  ...
  if (!grant.text || suppressed) {
    safe.openText=[];
    safe.words=[];
  } else {
    // Rebuild the client word cloud after media consent/grant filtering.
    // Reusing report.words here could retain transcript terms from media that
    // was correctly removed from safe.media above.
    const texts=safe.openText.map(item=>item.answer);
    if(grant.media)texts.push(...safe.media
      .filter(item=>['audio','video'].includes(item.media_type)&&item.transcript_status==='done'&&String(item.transcript_text||'').trim())
      .map(item=>item.transcript_text));
    safe.words=require('./studyReport').wordCloud(texts);
  }
  if (suppressed) {
