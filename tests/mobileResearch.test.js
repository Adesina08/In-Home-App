...
test('standard media preview is truthful when speech AI is unavailable and persists no preview row',async()=>{
 const beforeCount=await store.count('media');const form=new FormData();form.append('question_id','51');form.append('media',new Blob([Buffer.from('audio preview')],{type:'audio/mp4'}),'voice.m4a');const response=await fetch(url+`/mobile/api/respondents/${r.id}/diary/media-analysis`,{method:'POST',headers:headers(),body:form});assert.equal(response.status,200,await response.clone().text());const result=await response.json();assert.equal(result.questionId,51);assert.equal(result.transcriptStatus,'unavailable');assert.equal(result.scoreStatus,'unavailable');assert.equal(await store.count('media'),beforeCount);
});
test('standard audio submission stores its question link and completed analysis state',async()=>{
 const time=new Date().toISOString();const form=new FormData();for(const [key,value]of Object.entries({submission_id:'spoken_packet_12345',action:'submit',entry_mode:'standard',answers_json:JSON.stringify({'50':'A'}),occurrence_time:time,capture_time:time,submit_time:time}))form.append(key,value);form.append('audio_q_51',new Blob([Buffer.from('submitted audio')],{type:'audio/mp4'}),'voice.m4a');const response=await fetch(url+`/mobile/api/respondents/${r.id}/diary`,{method:'POST',headers:headers(),body:form});assert.equal(response.status,201,await response.clone().text());const receipt=await response.json();assert.equal(receipt.mediaResults[0].questionId,51);assert.equal(receipt.mediaResults[0].transcriptStatus,'unavailable');const saved=await store.findOne('media',{record_id:receipt.recordId,question_id:51});assert.equal(saved.media_type,'audio');assert.equal(saved.transcript_status,'unavailable');assert.equal(saved.transcript_score_status,'unavailable');
});
